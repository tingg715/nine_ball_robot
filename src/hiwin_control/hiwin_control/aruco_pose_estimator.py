    ...
    def pose_estimator(self):
        ...
        if ids is not None:
            ...
            for i, marker_id in enumerate(ids):
                # assign position fields
                self.pose.position.x = float(tvecs[i][0][0])
                self.pose.position.y = float(tvecs[i][0][1])
                self.pose.position.z = float(tvecs[i][0][2])

                rot_matrix, _ = cv2.Rodrigues(rvecs[i])
                quat = quaternion_from_matrix(rot_matrix)
                self.pose.orientation.x = float(quat[0])
                self.pose.orientation.y = float(quat[1])
                self.pose.orientation.z = float(quat[2])
                self.pose.orientation.w = float(quat[3])

                self.pose_array.poses.append(self.pose)
                # Int16MultiArray stores values in .data
                try:
                    self.marker_IDs.data.append(int(marker_id[0]))
                except Exception:
                    self.marker_IDs.data.append(int(marker_id))

                cv2.aruco.drawAxis(color_frame, self.camera_matrix, self.dist_coeffs, rvecs[i], tvecs[i], 0.1)

                print(f"Marker {ids[i][0]} position: X={tvecs[i][0][0]:.2f}, Y={tvecs[i][0][1]:.2f}, Z={tvecs[i][0][2]:.2f}")
